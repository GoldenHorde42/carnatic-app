# App Store & Google Play — Listing Content

Copy-paste these into App Store Connect and Google Play Console.

---

## App Name
`Carnatic`

## Subtitle (iOS only, 30 chars max)
`Classical music for students`

## Short Description (Google Play, 80 chars max)
`Discover Carnatic music by artist, raga, or mood. Powered by YouTube.`

---

## Full Description (used for both stores, ~4000 chars max)

Carnatic is the easiest way for music students, teachers, and enthusiasts to discover and enjoy Carnatic classical music — all in one curated, YouTube-powered app.

SEARCH THE WAY YOU THINK
Type anything: an artist's name, a raga, a tala, a composer, or even how you're feeling. Our intelligent search understands "Bhairavi raga on violin", "T.M. Krishna live concert", "Tyagaraja kritis", and even "I'm in a melancholic mood today."

CURATED FOR QUALITY
Unlike a generic YouTube search, Carnatic only shows you content from our hand-picked list of 66 verified Carnatic artists — no unrelated videos, no noise.

BROWSE BY ARTIST OR RAGA
Explore videos from your favourite artists or dive into a specific raga. All 72 melakarta ragas and 48 popular janya ragas are catalogued and searchable.

PERSONALISED RECOMMENDATIONS
Sign in with Google to get recommendations tailored to what you've been listening to. The more you listen, the smarter your feed becomes.

POWERED BY YOUTUBE
Watch full-length concerts, kutcheries, lecture demonstrations, and student performances — all streamed directly through YouTube's player.

FOR EVERYONE
- Students learning Carnatic music
- Teachers looking for repertoire examples
- Parents finding content for their children
- Enthusiasts who love classical South Indian music

FEATURES
- Natural language search (artist, raga, tala, mood, composer)
- Browse 66+ curated artists
- Full raga catalog: 72 melakartas + 48 janya ragas
- YouTube video player with native controls
- Personalised recommendations (free with Google sign-in)
- Dark mode interface
- Works without an account

Powered by YouTube. Content sourced via YouTube Data API v3.

---

## Keywords (iOS, 100 chars max — comma separated, no spaces after commas)
`carnatic,classical,music,raga,veena,violin,mridangam,flute,india,students,kutcheri`

## Category
- **iOS:** Music
- **Android:** Music & Audio

## Content Rating
- **iOS:** 4+ (no objectionable content)
- **Android:** Everyone

## Privacy Policy URL
`https://carnaticapp.org/privacy.html`

---

## What's New (Version 1.0)
First release of Carnatic — discover Carnatic classical music by artist, raga, or mood.
Powered by YouTube. Curated for students and enthusiasts.

---

## Screenshots needed (minimum)

### iOS
| Size | Device | Required |
|------|--------|----------|
| 1320×2868 (6.9") | iPhone 16 Pro Max | ✅ Yes (at least 2) |
| 1284×2778 (6.5") | iPhone 11 Pro Max | ✅ Yes (at least 2) |
| 2048×2732 (iPad 12.9") | iPad Pro | Optional |

### Android
| Size | Notes | Required |
|------|-------|----------|
| 1080×1920 or 9:16 | Phone screenshot | ✅ Yes (at least 2) |
| Feature graphic 1024×500 | Banner shown in Play Store | ✅ Yes |

### What to screenshot
1. **Home screen** — showing recommendations grid with mood chips
2. **Search screen** — showing results for an artist (e.g. "Bombay Jayashri")
3. **Browse screen** — showing artist list or raga list
4. **Video player** — YouTube player mid-playback with controls visible
5. **Search results** — showing mood/raga search results

### How to take screenshots
On phone during Expo Go testing:
- **iPhone:** Side button + Volume Up simultaneously → saves to Photos
- **Android:** Power + Volume Down simultaneously

Or use iOS Simulator in Xcode for perfect resolution:
```
Device & Simulator → choose iPhone 16 Pro Max → File → Take Screenshot
```

---

## App Store Connect — Required Fields Checklist

- [ ] App name: `Carnatic`
- [ ] Bundle ID: `com.carnaticapp.music`
- [ ] SKU: `carnaticapp2026`
- [ ] Primary language: English (U.S.)
- [ ] Category: Music
- [ ] Secondary category: Education (optional)
- [ ] Age rating: 4+
- [ ] Privacy Policy URL: `https://goldenhorde42.github.io/carnatic-app/privacy`
- [ ] Support URL: `https://goldenhorde42.github.io/carnatic-app` (or your email)
- [ ] Screenshots (min 2 per device size): see above
- [ ] Description: see above
- [ ] Keywords: see above
- [ ] What's New: see above
- [ ] Price: Free
- [ ] Distribution: All territories

---

## Google Play Console — Required Fields Checklist

- [ ] App name: `Carnatic`
- [ ] Package name: `com.carnaticapp.music`
- [ ] Short description: see above
- [ ] Full description: see above
- [ ] Screenshots (min 2): see above
- [ ] Feature graphic (1024×500): create in Canva
- [ ] App icon (512×512): same design as mobile icon
- [ ] Category: Music & Audio
- [ ] Content rating: complete questionnaire (answer "no" to everything)
- [ ] Privacy Policy URL: `https://goldenhorde42.github.io/carnatic-app/privacy`
- [ ] Data safety form: declare watch history collected, no data sold, data deletable
- [ ] Price: Free
- [ ] Countries: All available
